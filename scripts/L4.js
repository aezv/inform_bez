...
button_start_encrypt.addEventListener('click', function () {
    ...
    if (data_key.length) {
        ...
        let maxlength_encryption = Math.max(data_key.length, data_text.length);
        for (let i = 0; i < maxlength_encryption; i++) {
            let mi = alph.indexOf(data_text[((i >= data_text.length) ? i % data_text.length : i)]);
            let ki_s = data_key[((i >= data_key.length) ? i % data_key.length : i)];
            let ki = alph.indexOf(ki_s);
            let c = alph[((alph.length + (mi + ki)) % alph.length)];
            data_text_encryption.push(c);
        }
...
button_key_gen.addEventListener('click', function () {
    key.value = 'Генератор ключа не работает в этом методе шифрования';
});
